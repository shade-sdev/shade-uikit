...
export class EmployeesComponent {
  ...
  protected readonly managerLoadFn = (
    search: string,
  ): Observable<{ value: unknown; label: string }[]> => {
  ...
  protected readonly headcountStats = computed(() => {
    ...
    return [
      ...
      {
        label: 'Active',
        value: e.filter((x) => x.status === 'active').length,
        color: 'text-emerald-600',
      },
      {
        label: 'Remote',
        value: e.filter((x) => x.status === 'remote').length,
        color: 'text-blue-600',
      },
      {
        label: 'On Leave',
        value: e.filter((x) => x.status === 'on-leave').length,
  ...
  protected readonly columns: ColumnDef<Employee>[] = [
    ...
    },
    {
      ...
      sortable: true,
      cell: (row) =>
        `<span class="text-sm text-slate-600 dark:text-slate-400">${calcTenure(row.joinDate)}</span>`,
    },
